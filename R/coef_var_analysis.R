coef_var_analysis <- function(population, true_phi, M, sample_size, transform, iterations, numb_iterations, print_it = FALSE) {
  ...
  #       - numb_iterations: number of iterations for empirical distribution of estimated of variance
  ...
  variances_beta_prac <- c()
  variances_beta_prac_formula <- c()
  variances_beta_prac_formula2 <- c()

  variances_beta_theo <- c()
  variances_beta_theo_formula <- c()

  for (placeholder_index in 1:numb_iterations) {
  ...
  for (k in 1:iterations) {
    ...
    sample_theo <- population[sample(nrow(population), sample_size), ]
    ...
    variance_error_estimate_prac <- sum(error_estimate_prac^2) / (sample_size - ncol(X_sample_prac)) #adjust by degrees of freedom
    variance_beta_prac_formula <- 1/eigenvalues_hat_prac * variance_error_estimate_prac / (sample_size-1) #adjust in code because
    # the function PCR returns the eigenvalues of X'X/(n-1)
    ...
    variance_error_estimate_theo <- sum(error_estimate_theo^2) / ( (sample_size - ncol(X_sample_prac)))
    ...
    #evaluate results
    variances_beta_prac <- rbind(variances_beta_prac, colVars(store_beta_prac))
    variances_beta_prac_formula <- rbind(variances_beta_prac_formula, colMeans(store_variance_beta_prac_formula))
    variances_beta_prac_formula2 <- rbind( variances_beta_prac_formula2, colMeans(store_variance_beta_prac_formula2))

    variances_beta_theo <- rbind(variances_beta_theo, colVars(store_beta_theo))
    variances_beta_theo_formula <- rbind(variances_beta_theo_formula, colMeans(store_variance_beta_theo_formula))
  ...
  diff_Y_hat_changed <- colSums(abs(store_Y_hat_prac_changed - store_Y_hat_prac)) #means from every iteration
  diff_beta_hat_changed <- colSums(abs(store_beta_prac_changed - store_beta_prac))
...
coef_var_analysis_cl <- function(population, true_phi, M, sample_size, transform, iterations, print_it = FALSE) {
  #input: - population: matrix containing the y variable as first column and the regressors as the other columns
  #       - true_phi: maatrix of the true eigenvectors
  #       - M: number of principal components that should be used
  #       - sample_size: size of the samples used for the analysis
  ...
  for (placeholder_index in 1:1) {
    ...
    #evaluate results
    variances_beta_prac <- rbind(variances_beta_prac, store_beta_prac)
    variances_beta_prac_formula <- rbind(variances_beta_prac_formula, store_variance_beta_prac_formula)
    variances_beta_prac_formula2 <- rbind( variances_beta_prac_formula2, store_variance_beta_prac_formula2)

    variances_beta_theo <- rbind(variances_beta_theo, store_beta_theo)
    variances_beta_theo_formula <- rbind(variances_beta_theo_formula, store_variance_beta_theo_formula)
