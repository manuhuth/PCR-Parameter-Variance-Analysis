PCR <- function(Y, X, M, transform_Y = 'normalized', transform_X = 'normalized', EV_scal = 10, intercept = FALSE) {
  ...
  #transform Y and X
    if (transform_Y == 'normalized') { #demean Y
  ...
  list_return <- list('Y_hat' = Y_hat, 'resid' = residuals, 'mse' = mse, 'beta_Z' = beta_Z, 'Z' = Z, 'phi' = pca$phi, 'eigenval' = pca$eigenval, 'Y' = Y_untransformed, 'Y_transformed' = Y,
